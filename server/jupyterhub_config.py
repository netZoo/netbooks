...
c.ConfigurableHTTPProxy.command = '/opt/conda/envs/netbooks/bin/configurable-http-proxy'
# OAuth callback URL (must come before authenticator_class so subclass picks it up)
c.GithubOAuthenticator.oauth_callback_url = 'https://netbooks.networkmedicine.org/hub/oauth_callback'

# --- S3-cached allowed_users patch (replaces the eager S3 fetch above) ---
...
class S3CachedLocalGitHubOAuthenticator(LocalGitHubOAuthenticator):
    """LocalGitHubOAuthenticator that refreshes allowed_users from S3 with a TTL.

    Lets the admin update s3://netzoo/netbooks/netbooks_allowed_users.csv
    and have new users picked up within `_allowed_ttl_s` seconds, without
    restarting the hub.
    """
    _allowed_cache = {"ts": 0.0, "users": set()}
    _allowed_ttl_s = 300  # 5 minutes
    _s3_bucket = "netzoo"
    _s3_key = "netbooks/netbooks_allowed_users.csv"

    def _refresh_allowed(self):
        now = time.time()
        cache = self._allowed_cache
        if now - cache["ts"] < self._allowed_ttl_s and cache["users"]:
            return cache["users"]
        try:
            body = (
                boto3.client("s3")
                .get_object(Bucket=self._s3_bucket, Key=self._s3_key)["Body"]
                .read()
                .decode("utf-8")
            )
            df = pd.read_csv(StringIO(body), sep=",")
            users = set(df.users.dropna().astype(str).str.strip())
        except Exception as exc:  # noqa: BLE001
            self.log.warning(
                "S3 allowlist refresh failed (%s); keeping cached %d users",
                exc, len(cache["users"]),
            )
            return cache["users"]
        cache["ts"] = now
        cache["users"] = users
        self.log.info("Refreshed allowed_users from S3: %d users", len(users))
        return users

    async def check_allowed(self, username, auth_model):
        # Admins always pass
        if username in (self.admin_users or set()):
            return True
        allowed = self._refresh_allowed()
        # Match the previous behavior: only members of the CSV may log in
        return username in allowed

c.JupyterHub.authenticator_class = S3CachedLocalGitHubOAuthenticator
# Seed allowed_users from S3 once at startup so the admin UI shows them,
# but real enforcement happens in check_allowed (which re-reads on a TTL).
try:
    _seed = S3CachedLocalGitHubOAuthenticator()._refresh_allowed()
    c.Authenticator.allowed_users = set(_seed)
except Exception:
    pass
...
c.Spawner.default_url = 'notebooks/Welcome_to_netBooks.ipynb'
...
def create_dir_hook(spawner):
    ...
    tutorials_src = os.path.join('/home/ubuntu/netbooks', 'netbooks')
    tutorials_dest=os.path.join(volume_path)
    copyDirectory(tutorials_src, tutorials_dest)

# attach the hook function to the spawner
c.Spawner.pre_spawn_hook = create_dir_hook
c.Spawner.cmd = '/opt/conda/envs/netbooks/bin/jupyterhub-singleuser'
...
#  Default: '/opt/conda/envs/netbooks/share/jupyterhub'
# c.JupyterHub.data_files_path = '/opt/conda/envs/netbooks/share/jupyterhub'
